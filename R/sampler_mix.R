...
mcmcMix = function(y,X,model,prior=list(),mcmc){

  defaultPrior=list(e0=0.01,p=1000,hyperprior=F)
  prior=utils::modifyList(defaultPrior, as.list(prior))

  prior_par=prior
  prior=createPrior(y,X,model,prior_par$p)
  ...
  #slightly regularised regression coefficient beta_0
  cholx = chol(XX + diag(c(psi_inv_itcpt,rep(psi_con_inv,n_con),rep(0.01,jk)) ) )
  BN =  backsolve(cholx, backsolve(cholx, diag(ncol(cholx)),transpose=TRUE))
  beta_0 = BN %*% Xy
  beta=beta_0
  sgma2=sum((y-X%*%beta_0)^2)/(N-r)

  #####initial value for allocation vector S:
  S=rep(0,jk)   #indicator vector for beta_jk

  #####initial value for the  vector reporting the number of effects allocated to a specific component
  N_jl=rep(1,Lall)
  N_j0=rep(0,n_nom)

  cholx = chol(XX)
  beta_ols=  backsolve(cholx, backsolve(cholx, diag(ncol(cholx)),transpose=TRUE))%*% crossprod(X,y)
  comp_means=(beta_ols)[(r-jk+1):r]
	mu=comp_means
	mean_beta_jl=(beta_ols)[(r-jk+1):r]
  ...
  result<-list(
    beta=matrix(0,M_thin,r),
    sgma2=rep(0,M_thin),
    S=matrix(0,M_thin,jk),  # oder doch mit intercept ?
    eta=matrix(0,M_thin,Lall),
    eta0=matrix(0,M_thin,n_nom),
    mu = matrix(0,M_thin,Lall),
    N_jl_matrix=matrix(0,M_thin,Lall),
    N_j0_matrix=matrix(0,M_thin,n_nom),
    prior=prior
  )
  ...
  m_thin=1
    ...
    if(m%%500==0)  cat("finite mixture: ",m,"\n")
    ...
    b0=c(0,rep(mu_con,n_con),comp_means) ## component means
    ...
    if(thinning>1 & mcmc$burnin==0){
      if(m%%thinning==0) result$beta[m_thin,]=beta
    } else {
      result$beta[m,]=beta
    }
    ...
    if(thinning>1 & mcmc$burnin==0 & (m%%thinning==0)){
      if(m%%thinning==0) result$sgma2[m_thin]=sgma2
    } else {
    ...
    if(thinning>1 & mcmc$burnin==0){
      if(m%%thinning==0) result$mu[m_thin,]=mu
    } else {
      result$mu[m,]=mu
    }
    ...
    if(prior_par$hyperprior){
     ...
     for (j in (1:n_nom)){
      ...
      for(l in 1:(L[j]+1)){    #for each mixture component l, with 0-component
        if(Nh_j[indc_j][l]>0){
    ...
    for (j in (1:n_nom)){
      ...
      mat= sapply(Lj1_vec,function(l) log(eta_j[l])+stats::dnorm(betaj,mu_j[l],sq_psi_j[l],log=T))
      ...
      if(sum(is.na(mean.betah_j))>0){               ##to catch the case if a group is empty: NA values are substituted by zeros
