...
mcmcMixLogit <- function(y, X, model, prior = list(), mcmc, returnBurnin) {
    ...
    result <- list(beta = matrix(NA, M_thin + burnin, r), S = matrix(NA, M_thin + burnin, jk), 
        eta = matrix(NA, M_thin + burnin, Lall), eta0 = matrix(NA, M_thin + burnin, n_nom), mu = matrix(NA, 
            M_thin + burnin, Lall), N_jl_matrix = matrix(NA, M_thin + burnin, Lall), N_j0_matrix = matrix(NA, 
            M_thin + burnin, n_nom))
    ...
    for (m in 1:(M + burnin)) {
        ...
        BN <- tryCatch(chol2inv(Matrix::chol(B0_inv + Matrix::crossprod(X, Omega %*% X))), error = function(e) {
            solve(B0_inv + Matrix::crossprod(X, Omega %*% X))
        }, B0_inv = B0_inv, X = X, Omega = Omega)
